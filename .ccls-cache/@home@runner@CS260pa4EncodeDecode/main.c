...
// TODO: Implement a binary search tree (BST) data structure to be used in the
...
// This is the bare minimum you need for implementing a priority queue and BST
...
void	deleteBinarySearchTree( BST *bst );
...
void encode( FILE *plaintext, FILE *codetable, FILE *encodetxt ) {
	...
	//       text in the previous section, by using the priority queue and BST
	//       you implemented
...
void decode( FILE *codetable, FILE *encodetxt, FILE *decodetxt ) {
	...
	int		ce, cd;      // Temporary int/char used for iteration
	float	f;           // A temporary float variable used for iteration
